...
class SRSGetter {
 public:
  ...
  bool isMip() { return prob()->is_mip; }

  int getNumVariables() { return SRSgetnbcols(prob()); }
  ...
  int getScaling() { return prob()->scaling; }

  double getRelativeMipGap() { return prob()->relativeGap; }
 ...
 private:
  ...
  SRS_PROBLEM* prob() { return (SRS_PROBLEM*)solver_->underlying_solver(); }
};
...
void _unittest_verify_var(SRSGetter* getter, MPVariable* x, int type, double lb,
                          double ub) {
...
void _unittest_verify_constraint(SRSGetter* getter, MPConstraint* c, char type,
                                 double lb, double ub) {
  int idx = c->index();
...
TEST(TestSiriusInterface, setVarBoundType) {
  ...
  std::array<int, 5> varBoundTypes = {
      VARIABLE_FIXE, VARIABLE_BORNEE_DES_DEUX_COTES,
      VARIABLE_BORNEE_INFERIEUREMENT, VARIABLE_BORNEE_SUPERIEUREMENT,
      VARIABLE_NON_BORNEE};
